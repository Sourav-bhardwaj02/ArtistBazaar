...
export function ServiceDashboardHeader() {
  ...
  const { id } = useParams(); // ✅ Pure URL param - NO localStorage
  ...
  useEffect(() => {
    const checkAuth = () => {
      ...
      if (token && userData && id) { // ✅ Uses URL id
        try {
          const parsedUser = JSON.parse(userData);
          if (parsedUser.role === "Services") {
            setUser({
              name: parsedUser.name || "Service",
              role: "Services",
            });
          } else {
            setUser(null);
  ...
  }, [id, navigate]); // ✅ URL id dependency

  const handleLogout = () => {
    localStorage.removeItem("auth-token");
    localStorage.removeItem("user-data");
    // ✅ NO servicesId - pure services routes
    setUser(null);
    ...
    <header className="h-16 bg-card/50 backdrop-blur border-b border-border/50 flex items-center justify-between px-6">
      <div className="flex items-center gap-4">
        ...
        <div className="relative w-80">
          ...
          <Input
            placeholder="Search services, chats, analytics..."
      ...
      <div className="flex items-center gap-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="relative h-10 w-10 rounded-full" aria-label="Service menu">
              <Avatar className="h-10 w-10">
                <AvatarImage src="/placeholder.svg" alt={user.name || "Service"} />
          ...
          <DropdownMenuContent className="w-48 bg-card/90 backdrop-blur-md text-foreground" align="end">
            ...
            <DropdownMenuItem asChild>
              <Link to={`/services/${id}/dashboard`} className="flex items-center w-full">
